...
class homeController {
  ...
  get_products = async (req, res) => {
    try {
      // Lấy ra danh sách sản phẩm tối đa 20 sản phẩm và sắp xếp theo thời gian được tạo
      const products = await productModel
        .find({})
        .limit(20)
        .sort({ createAt: -1 });

      // Lấy ra danh sách sản phẩm tối đa 9 sản phẩm và sắp xếp theo thời gian được tạo
      const allProduct1 = await productModel.find({}).limit(9).sort({
        createdAt: -1,
      });
      const latest_product = this.formateProduct(allProduct1);

      // Lấy ra danh sách sản phẩm tối đa 9 sản phẩm và sắp xếp theo đánh giá
      const allProduct2 = await productModel.find({}).limit(9).sort({
        rating: -1,
      });
      const topRating_product = this.formateProduct(allProduct2);

      // Lấy ra danh sách sản phẩm tối đa 9 sản phẩm và sắp xếp theo mã giảm giá
